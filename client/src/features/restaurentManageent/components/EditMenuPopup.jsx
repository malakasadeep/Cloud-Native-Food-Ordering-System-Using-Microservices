...
import { storage } from "../../../core/utils/firebaseStorage";
...
const EditMenuPopup = ({
  ...
  // Reset form when menuData changes (when editing a different item)
  useEffect(() => {
    if (menuData) {
      setFormData({
        name: menuData.name || "",
        description: menuData.description || "",
        price: menuData.price !== undefined ? String(menuData.price) : "",
        categoryId: menuData.categoryId || "",
        categoryName: menuData.categoryName || "",
        availability:
          menuData.availability !== undefined ? menuData.availability : true,
  ...
  // Fetch categories when the component mounts or restaurantId changes
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await menuService.getCategoryByRestaurantId(restaurantId);
        if (res.success) {
          // Ensure that we're setting an array to categories
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        onUpdate(response.data); // Pass the updated data back to parent
            ...
            >
              ...
              <div className="flex flex-col space-y-4">
                <div
                  className={`relative flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg transition-all duration-300 h-80
                    ${
                      dragOver
                        ? "border-cartNumBg bg-red-50"
                        : "border-lighttextGray hover:border-cartNumBg"
                    }
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div>
                  <label
                    htmlFor="name"
                    className="block mb-2 text-sm font-medium text-headingColor"
                  >
                    Item Name *
                  </label>
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-card border rounded-md focus:outline-none focus:ring-2 transition-all ${
                      errors.name
                        ? "border-cartNumBg focus:ring-cartNumBg"
                        : "border-lighttextGray focus:ring-cartBg"
                    }`}
                ...
                <div className="flex items-center py-2">
                  ...
                  <label
                    ...
                    className="block ml-2 text-sm text-textColor"
                  >
              ...
              <div className="flex justify-end gap-3 pt-4 mt-2 border-t border-gray-200 col-span-1 md:col-span-2">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className={`flex items-center px-6 py-2.5 text-sm text-white transition-colors ${
                    isSubmitting ? "bg-gray-400" : "bg-cartBg hover:bg-black"
                  } rounded-md focus:outline-none focus:ring-2 focus:ring-cartNumBg`}
                >
