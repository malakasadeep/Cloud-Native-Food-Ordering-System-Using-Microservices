...
const MenuTable = ({ restaurantId, onEdit, onDelete }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch menu items
        const menuResponse = await menuService.getMenuByRestaurantId(restaurantId);
        ...
        // Fetch categories
        const categoriesResponse = await menuService.getCategoryByRestaurantId(restaurantId);
  ...
  if (error) {
    return (
      <div className="bg-red-50 p-4 rounded-md text-red-600">
        <p className="flex items-center"><Info className="mr-2" size={20} /> {error}</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Image</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Name</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Description</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Price</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Category</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Availability</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-headingColor uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((item) => (
                <tr key={item._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-headingColor font-medium">{item.name}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-textColor max-w-xs truncate">{item.description || 'No description'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-headingColor font-medium">${item.price.toFixed(2)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-textColor">{item.categoryName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input 
                        ...
                        onChange={() => handleAvailabilityToggle(item._id, item.availability)} 
              ...
              <tr>
                <td colSpan="7" className="px-6 py-4 text-center text-sm text-textColor">
