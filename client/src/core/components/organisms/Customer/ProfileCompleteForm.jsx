...
import { Check, MapPin, CreditCard, ChevronLeft, ChevronRight, Plus, Trash2 } from 'lucide-react';
...
const ProfileCompleteForm = ({ initialData = {}, onSubmit, onSkip }) => {
  ...
  useEffect(() => {
    if (initialData.location && initialData.location.latitude && initialData.location.longitude) return;
      ...
      (error) => {
        ...
        switch(error.code) {
          ...
          case error.POSITION_UNAVAILABLE:
            setLocationError("Location information is unavailable at this time.");
            break;
          case error.TIMEOUT:
            setLocationError("Request to get location timed out. Please try again.");
            break;
          default:
            setLocationError("An unknown error occurred while trying to retrieve your location.");
            break;
        }
  ...
  const removeSecondaryAddress = (id) => {
    setFormData(prev => ({
      ...prev,
      secondaryAddresses: prev.secondaryAddresses.filter(addr => addr.id !== id)
    }));
  };
  ...
  const addPaymentMethod = () => {
    setFormData(prev => ({
      ...
      paymentMethods: [
        ...prev.paymentMethods,
        { id: Date.now(), type: 'card', cardNumber: '', cardName: '', expiry: '', cvv: '' }
      ]
    }));
  };
  ...
  const steps = [
    { 
      title: 'Basic Info',
      icon: <div className="w-6 h-6 rounded-full bg-cartNumBg text-white flex items-center justify-center text-xs">1</div>,
      content: (
        ...
        <div className="space-y-3" key="location-step">
          ...
          <div className="h-[320px] rounded-lg overflow-hidden border border-gray-300 relative">
            <MapComponent
              ...
              mapContainerStyle={{...mapContainerStyle, height: "320px", minHeight: "320px"}}
            />
          </div>
          <div className="p-2 bg-blue-50 rounded-md text-xs">
            <p className="text-gray-700">
              <strong>Coordinates:</strong> Lat: {(formData.location?.latitude || 0).toFixed(6)}, 
              Lng: {(formData.location?.longitude || 0).toFixed(6)}
            </p>
        ...
        <div className="space-y-4 overflow-auto max-h-[320px] pr-1">
          <div>
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-sm font-medium text-gray-900">Secondary Addresses (Optional)</h3>
              <button
                type="button"
            ...
            {formData.secondaryAddresses.length === 0 ? (
              <p className="text-xs text-gray-500 italic">No secondary addresses added</p>
            ) : (
              <div className="space-y-2">
                {formData.secondaryAddresses.map((address, index) => (
                  <div key={address.id} className="p-2 border border-gray-200 rounded-md">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-xs font-medium">Address {index + 1}</h4>
                      <button
                        type="button"
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <input
                          type="text"
                          value={address.name}
                          onChange={(e) => updateSecondaryAddress(address.id, 'name', e.target.value)}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateSecondaryAddress(address.id, 'address', e.target.value)}
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-sm font-medium text-gray-900">Payment Methods (Optional)</h3>
              <button
                type="button"
            ...
            {formData.paymentMethods.length === 0 ? (
              <p className="text-xs text-gray-500 italic">No payment methods added</p>
            ) : (
              <div className="space-y-2">
                {formData.paymentMethods.map((method, index) => (
                  <div key={method.id} className="p-2 border border-gray-200 rounded-md">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-xs font-medium">Payment Method {index + 1}</h4>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updatePaymentMethod(method.id, 'cardName', e.target.value)}
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={method.expiry}
                          onChange={(e) => updatePaymentMethod(method.id, 'expiry', e.target.value)}
                          placeholder="MM/YY"
                          className="w-full px-2 py-1 text-xs border border-gray-300 rounded-md"
                        />
                      </div>
  ...
  const nextStep = () => {
    if (validateStep(currentStep)) {
      ...
      if (currentStep === 1) {
        console.log("Moving from map to final step - ensuring no auto-submission");
    ...
    <div className="h-[70vh] max-h-[600px] flex flex-col p-3">
        ...
        {steps.map((step, index) => (
          <React.Fragment key={index}>
            <div className="flex items-center">
              ...
              <span className="ml-1 text-xs font-medium hidden sm:inline">{step.title}</span>
            </div>
            {index < steps.length - 1 && (
              <div 
                className={`flex-1 h-[2px] mx-1 ${
                  index < currentStep ? 'bg-gradient-to-r from-cartNumBg to-green-500' : 'bg-gray-200'
                }`}
              />
            )}
          </React.Fragment>
      ...
      >
        ...
        <div className="flex justify-between">
          {currentStep > 0 ? (
            <motion.div variants={buttonVariants} whileHover="hover" whileTap="tap" className="w-[48%]">
              <Button 
                type="button" 
                variant="outline" 
                onClick={prevStep}
          ...
          {currentStep < steps.length - 1 ? (
            ...
            <motion.div variants={buttonVariants} whileHover={isSubmitting ? {} : "hover"} whileTap={isSubmitting ? {} : "tap"} className="w-[48%]">
              <Button 
                type="submit"
                variant="primary"
                icon={isSubmitting ? null : Check}
                disabled={isSubmitting}
                className={`w-full h-8 text-xs ${isSubmitting ? "bg-green-500" : "bg-cartNumBg hover:bg-orange-600"} text-white`}
              >
                {isSubmitting ? (
